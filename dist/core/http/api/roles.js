...
function registerRoleRoutes(app, prefix = '') {
    const p = prefix;
    ...
    app.route({
        ...
        handler: async ({ body, req }) => {
            const schema = zod_1.z.object({
                ...
                contextType: zod_1.z.string().min(1),
    ...
    app.route({
        ...
        handler: async ({ params, req }) => {
            try {
                const { name } = zod_1.z.object({ name: zod_1.z.string().min(1) }).parse(params);
    ...
    app.route({
        ...
        handler: async ({ body, req }) => {
            const schema = zod_1.z.object({
                roleName: zod_1.z.string().min(1).optional(),
                roleKey: zod_1.z.string().optional(),
    ...
    app.route({
        ...
        handler: async ({ params, body, req }) => {
            ...
            const parsed = schema.safeParse(body);
            logger_1.logger.log('Validation result:', {
        ...
        ...(app.authzEnabled
            ? [
                (req, res, next) => {
                    const { contextType } = req.body;
                    return app
                        .authorize(policy.roles.removePerm.roleManage.replace('{type}', contextType), { scope: 'type-wide', contextType: 'required' })(req, res, next);
                },
                (req, res, next) => {
