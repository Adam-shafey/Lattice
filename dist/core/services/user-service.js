...
class UserService extends base_service_1.BaseService {
    ...
    async createUser(params) {
        ...
        if (password.length < 8) {
            throw base_service_1.ServiceError.validationError('Password must be at least 8 characters long');
        }
        return this.execute(async () => {
            // Check if user with email already exists
            const existing = await this.db.user.findUnique({ where: { email } });
            if (existing) {
                throw base_service_1.ServiceError.conflict(`User with email '${email}' already exists`);
            }
            // Hash the password
    ...
    async updateUser(id, updates, context) {
        ...
        return this.execute(async () => {
            ...
            if (!existing) {
                throw base_service_1.ServiceError.notFound('User', id);
            }
            // Check if email is being changed and if it already exists
            if (updates.email && updates.email !== existing.email) {
                const emailExists = await this.db.user.findUnique({ where: { email: updates.email } });
                if (emailExists) {
                    throw base_service_1.ServiceError.conflict(`User with email '${updates.email}' already exists`);
                }
            }
            // Prepare update data
            const updateData = {};
            ...
            if (updates.password) {
                updateData.passwordHash = await (0, bcryptjs_1.hash)(updates.password, 12);
            }
            // Update the user
    ...
    async changePassword(userId, oldPassword, newPassword, context) {
        ...
        if (newPassword.length < 8) {
            throw base_service_1.ServiceError.validationError('New password must be at least 8 characters long');
        }
        return this.execute(async () => {
            ...
            // Verify old password
            const isOldPasswordValid = await (0, bcryptjs_1.compare)(oldPassword, user.passwordHash);
            if (!isOldPasswordValid) {
                throw base_service_1.ServiceError.unauthorized('Current password is incorrect');
            }
            // Hash new password
